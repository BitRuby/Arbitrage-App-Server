...
exports.init = function(app) {
    ...
    app.route('/api/binance/summary/:c1/:c2').get((req, res) => {
        const c1 = req.params.c1.toUpperCase(); 
        const c2 = req.params.c2.toUpperCase();
        Request.get( {url:`https://api.binance.com/api/v1/ticker/24hr?symbol=${c2}${c1}`, headers: {'User-Agent': 'request'}},  (error, response, body) => {
            if(error) {
                res.send(error);
    ...
    app.route('/api/okex/summary/:c1/:c2').get((req, res) => {
        let c1 = req.params.c1.toUpperCase(); 
        let c2 = req.params.c2.toUpperCase();
        Request.get( {url:`https://www.okex.com/api/spot/v3/instruments/${c2}-${c1}/ticker`, headers: {'User-Agent': 'request'}},  (error, response, body) => {
            if(error) {
                res.send(error);
                return console.dir('HitBTC summary: ' + error);  
            }
